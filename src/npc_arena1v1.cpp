...
public:
    ...
    struct npc_1v1arenaAI : public ScriptedAI
    {
        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
        {
            ...
            case 8: // Script Info
            {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Developer: MDic", GOSSIP_SENDER_MAIN, action);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Group Single Player Project", GOSSIP_SENDER_MAIN, action);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "http://wow.opiums.eu/dc", GOSSIP_SENDER_MAIN, action);
